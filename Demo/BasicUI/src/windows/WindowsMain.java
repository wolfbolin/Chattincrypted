...
public class WindowsMain extends Application {
    ...
    private WindowsFind wFind = new WindowsFind();
    ...
    private HBox NewhbBottom = new HBox();
    ...
    private Button btFind = new Button("查找");
    ...
    private void InitTab() {
        ...
        spTabBasic.setContent(lvContacts);
    ...
    //鼠标点击打开查询窗口事件
    /*private void openWindow(MouseEvent e, WindowsFind wf) {
    	if (e.getClickCount() >= 2) {
    		if (!wf.isShowing()) wf.show();
    		else wf.requestFocus();
    	}
    }*/
    ...
    private void InitBottomButton() {
        ...
        NewhbBottom.setStyle("-fx-background-color: #d1d1d1;");
        hbBottom.setAlignment(Pos.BOTTOM_LEFT);
        NewhbBottom.setAlignment(Pos.BOTTOM_LEFT);
        NewhbBottom.setMinWidth(280);
        NewhbBottom.setMaxWidth(280);
        NewhbBottom.setMinHeight(50);
        NewhbBottom.setMaxHeight(50);
        hbBottom.getChildren().addAll(ivButtomBt);
        NewhbBottom.getChildren().addAll(hbBottom,btFind);
        
    }

    /** TODO: 初始化面板 */
    private void InitPane(Stage primaryStage) {
        root.getChildren().addAll(hbLogo, hbHeadIcon, tfSearch, hbTab, spTabBasic, NewhbBottom);
        ...
        wLogin.show();
        wLogin.getBtFind().setOnAction(e -> {
            primaryStage.show();
